...
def custom_model_training(df,st):
    ...
    for train, test in kfold.split(x_scale, y):
        ...
        y_pred=model_1.predict(x_scale[test])
        y_pred=pd.DataFrame(y_pred).apply(lambda x:np.round(x).astype(int))
        cm=confusion_matrix(y.iloc[test].values,y_pred)
        #st.write(cm)
        confusion_matrix_dataframe=pd.DataFrame(cm,columns=["Not Admitted","Admitted"],index=["Not Admitted","Admitted"])
    
        
        sns.heatmap(confusion_matrix_dataframe,annot=True,square=True)
        
        plt.xlabel('Predicted')
        plt.ylabel('Truth')
        plt.title(f"Confusion Matrix at iteration {iteration_counter}")
        st.pyplot(plt.show())
