...
def test():
    try:
        ...
        if uploaded_file is not None:    
            ...
            ad=pd.DataFrame(c).apply(lambda x:np.round(x).astype(int))
            st.markdown(f"<h5 style='text-align: center; color:grey'>Total number of students that are predicted to take admission are-:{len(ad[0]==1)} out of {len(ad)}</h5>", unsafe_allow_html=True)
            ...
            for i in sections:
                if st.checkbox(i):
                    ...
                    temp_df['Admitted']=pd.DataFrame(c).apply(lambda x:np.round(x).astype(int))
                    wanted_df=temp_df.loc[temp_df['Admitted']==1]
